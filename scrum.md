...
- [ ] Each M0 question in `questions.md` (host OS, filesystem, ZFS vs MinIO, managed paths under `/mnt/ext`) has a written decision in `docs/decisions/` with context, decision, alternatives, consequences, and date.
- [ ] `README.md` is updated to reference each decision.
- [ ] The resolved questions are removed from `questions.md`.
...
- [ ] Root `Cargo.toml` declares a workspace with members: `crates/api`, `crates/worker`, `crates/domain`, `crates/storage`, `crates/db`, `crates/media`, `crates/importer`.
- [ ] Each crate compiles (`cargo check`) as an empty skeleton with appropriate `lib.rs` or `main.rs`.
- [ ] `crates/api` and `crates/worker` are binary crates; the rest are library crates.
- [ ] `cargo fmt --check` and `cargo clippy` pass with zero warnings.
- [ ] A `.cargo/config.toml` is present for cross-compilation targets if needed (ARM64 + x86-64).
- [ ] `cargo build --target aarch64-unknown-linux-gnu` succeeds (or documents the exact cross-compile setup).
...
- [ ] `apps/web/` contains a Vite + SolidJS + TypeScript project scaffolded with `degit` or `create vite`.
...
- [ ] ESLint and Prettier are configured and pass on the initial codebase.
- [ ] TypeScript `strict` mode is enabled in `tsconfig.json`.
- [ ] All fonts and assets are locally bundled — **zero** references to external CDNs.
...
- [ ] `docker compose -f docker-compose.dev.yml up` starts PostgreSQL 16+ and Apache Tika.
- [ ] PostgreSQL is accessible on `localhost:5432` with a dev database, user, and password configured via `.env.example`.
- [ ] Tika is accessible on `localhost:9998`.
- [ ] A `volumes` section persists PostgreSQL data across restarts.
- [ ] Images used are available for both `linux/arm64` and `linux/amd64`.
- [ ] `.env.example` documents every required environment variable.
- [ ] `docker compose down -v` cleanly removes volumes for a fresh start.
...
- [ ] `crates/db/migrations/` contains a `00000000000000_init.sql` (or similar) that creates the database extensions (e.g., `uuid-ossp` or `pgcrypto`).
- [ ] `cargo sqlx migrate run` applies migrations to the dev database.
- [ ] `cargo sqlx migrate revert` rolls back the latest migration.
- [ ] A `DATABASE_URL` env var is the sole connection config, documented in `.env.example`.
- [ ] SQLx offline mode (`sqlx-data.json` / `cargo sqlx prepare`) is set up so CI builds work without a live database.
...
- [ ] `crates/api` reads `DATABASE_URL`, `STORAGE_ROOT`, `LISTEN_ADDR`, and `TIKA_URL` from environment variables.
- [ ] On startup, the API: connects to PostgreSQL, verifies `STORAGE_ROOT` exists and is writable, and logs a structured JSON startup message.
- [ ] If PostgreSQL is unreachable, the API exits with a non-zero code and a clear error message within 5 seconds.
- [ ] If `STORAGE_ROOT` is missing or unwritable, the API exits with a non-zero code and a clear error message.
- [ ] Structured JSON logging is used for all log output (e.g., `tracing` + `tracing-subscriber` with JSON formatter).
- [ ] The binary does **not** hard-code any paths under `/mnt/ext`; all paths are configurable.
...
- [ ] `GET /health` returns `200 OK` with `{"status": "ok"}` if the API process is running.
- [ ] `GET /ready` returns `200 OK` with a JSON body including: `postgres: "ok" | "error"`, `storage: "ok" | "error"`, `tika: "ok" | "error"`, `disk_usage_percent: number`.
- [ ] `GET /ready` returns `503 Service Unavailable` if any dependency check fails.
- [ ] Each dependency check has a timeout (≤ 2s) so `/ready` doesn't hang.
- [ ] Tests cover the healthy and degraded paths.
...
- [ ] A typed API client module exists at `apps/web/src/api/client.ts` using `fetch`.
- [ ] The Vite dev server proxies `/api/*` to the Axum API (configured in `vite.config.ts`).
- [ ] The placeholder page calls `GET /api/ready` on mount and displays the connection status.
- [ ] If the API is unreachable, a user-friendly error is shown (not a raw exception).
- [ ] The API client is typed: request/response types are defined in a shared `types.ts`.
...
- [ ] A `tokens.css` (or equivalent) defines CSS custom properties under `:root` (light) and `[data-theme="dark"]` (dark).
- [ ] Dark theme uses true-black (`#000`) background per plan spec.
- [ ] Token categories include: surface colors, text colors, border colors, accent/brand colors, spacing scale, font sizes, font weights, border radii, shadows.
- [ ] Fonts are locally hosted (downloaded into `apps/web/public/fonts/`) with `@font-face` declarations — no Google Fonts CDN links.
- [ ] A `<ThemeProvider>` or `data-theme` toggle mechanism exists and persists the user's choice to `localStorage`.
- [ ] A dev-only "theme preview" page or component renders sample swatches for visual verification.
...
- [ ] A CI config file (GitHub Actions `.github/workflows/ci.yml` or equivalent) is present.
- [ ] CI runs: `cargo fmt --check`, `cargo clippy -- -D warnings`, `cargo build`, `cargo test`, `npm run lint`, `npm run build` (in `apps/web/`).
- [ ] CI runs on both `push` and `pull_request` to `main`.
- [ ] CI includes a matrix for `x86_64` (and optionally `aarch64` if runner is available).
- [ ] A `Makefile`, `justfile`, or equivalent defines `check`, `lint`, `build`, `test` targets for local dev.
