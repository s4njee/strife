...
**Full-set promotion (2026-08-04):** The same frozen campaign passed the 1,000- and 10,000-file gates and recorded an approved 10,000-file result: 9,744 completed, 238 skipped, and 18 retained terminal failures, with a 0.862-second median, 5.674-second p95, 501,792,768-byte peak sampled OCR process memory, and 53.2 C peak sampled temperature. Cancellation during PDF rasterization had exposed temporary-page leakage, so revision `d7ff7a0` now constructs the managed temporary-page guard before rasterization and proves cleanup with a cancellation regression test. Orion activated the immutable ARM64 `d7ff7a0` release, reset the worker tmpfs from 655 MiB used to empty, and resumed the campaign without a canary limit at an audited two-job running limit. Initial full-run verification showed 10,137 files enqueued, two leased jobs, 30,801 candidates remaining, a healthy API/PostgreSQL/Tika stack, and 224 KiB of 768 MiB worker tmpfs in use. Email backfill remains unstarted until OCR completes.
