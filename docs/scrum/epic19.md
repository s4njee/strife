...
**The production-scale run has not been performed, and three criteria stay open.** Cold and warm latency percentiles, `EXPLAIN (ANALYZE, BUFFERS)` plans, and ingestion and index-growth figures all require executing the harness on Orion against ≥100,000 messages. Running it now would measure a moving target — the parser, ranking weights, and filter set have been stable for hours, not through a canary — and the numbers would be re-measured anyway. The benchmark belongs immediately before the Phase 7 email canaries in [`backfill.md`](../backfill.md), alongside Story 22.5's validation, where its thresholds actually gate a decision. The document says so in a status callout rather than reading as though the evidence exists.
